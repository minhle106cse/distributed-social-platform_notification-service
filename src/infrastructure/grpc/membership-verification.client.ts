...
/**
 * Client side of proto/membership.proto — notification-service has no
 * Membership table of its own, so a caller-supplied X-Org-Id must be
 * verified against core-api before being trusted (IDOR-class fix,
 * resilience_patterns.md). Same convention as search-service's client of
 * the same name.
 */
@Injectable()
